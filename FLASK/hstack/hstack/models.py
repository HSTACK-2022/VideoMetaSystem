...
class CorePost(db.Model):
    ...
    author_id = db.Column(db.ForeignKey('auth_user.id'), index=True)
    category_id = db.Column(db.ForeignKey('core_category.id'), index=True)

    author = db.relationship(
        'AuthUser', primaryjoin='CorePost.author_id == AuthUser.id', backref='core_posts')
...
class Videopath(db.Model):
    ...
    videoAddr = db.Column(db.String(100))
