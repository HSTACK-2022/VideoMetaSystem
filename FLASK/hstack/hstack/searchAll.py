...
class Total:
    ...
    def getVideoMetadataFromID(self, videoId):
        ...
        keywordList = list()
        # list형태
        for kwlist in DB.session.query(Keyword).filter(keywordQ).with_entities(Keyword.keyword).all():
            for kw in kwlist:
        ...
        if OS == 'Windows':
            filePath = DB.session.query(Videopath).filter(
        ...
        count = 0
        imageList = os.listdir(DB.session.query(Videopath).filter(
            Videopath.id == videoId).first().imageAddr)
...
def extractType(videoIdList):
    ...
    for videoId in videoIdList:
        if (DB.session.query(Metadatum).filter(Metadatum.id == videoId).first().narrative):
            types = DB.session.query(Metadatum).filter(
...
def extractCategories(videoIdList):
    ...
    for videoId in videoIdList:
        if (DB.session.query(Metadatum).filter(Metadatum.id == videoId).first().category):
            category = DB.session.query(Metadatum).filter(
...
def extractData(videoIdList):
    ...
    for videoId in videoIdList:
        if (DB.session.query(Metadatum).filter(Metadatum.id == videoId).first().method):
            datas = DB.session.query(Metadatum).filter(
...
def search(All, T, K, P):
    ...
    print("^^^^^^^^^^^")
    print(in_perc)
    # {80: [0, 0, 0], 21: [0, 0, 0.567], 22: [0, 0, 0.567], 77: [0, 0, 0.058]}
    print(perc)
    # ranking 결과
    print(weight)
